...
#import re  # 导入正则表达式，在本项目中用于解析Github文档的markdown格式内容
#import webbrowser  # 导入web浏览器模块，在本项目中用户打开一个web页面展示翻译后的文档内存
#import tempfile  # 导入临时文件模块，在本项目中用于创建一个临时问文件夹
#import markdown  # 导入markdown库，在本项目中用于格式化文档的内容并显示
